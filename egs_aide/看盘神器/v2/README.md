...
5. Excel 配置      在 Excel 里改配置，不用动代码，不用重启
```
...
1. 安装依赖：
    ...
    主要依赖：`xlwings`（操作 Excel）、`qstock`（行情数据）、`mplfinance`（K线图）、`pyyaml`（配置）、`openpyxl`（生成模板）
...
2. 生成 Excel 模板（首次运行）：
    ...
    程序会自动生成 `看盘模板.xlsx`，里面已经带好了 5 个 Sheet 和示例自选股，开箱即用。
...
main.py                         主入口（加载配置 → 初始化 → 刷新循环）
  ...
  └── src/excel_monitor/
        ...
        │     ├── data_provider.py       数据源封装（qstock + 统一异常处理）
...
和 V1 一样，主要采用 `qstock` 库获取数据，qstock 主要使用东方财富和同花顺的免费接口。
...
V2 把所有数据获取封装进 `DataProvider`，统一异常处理，单个接口失败不会影响其他数据：
...
| 异常处理 | 基本无 | 统一封装，日志清晰 |
| 数据源封装 | 直接调 qstock | DataProvider 统一封装 + 异常兜底 |
| 盘口异动 | 未启用 | 已启用（详细行情 Sheet） |
| 单元测试 | 无 | pytest 覆盖核心逻辑 |
| 刷新稳定性 | 偶发崩溃 | 容错强，可长时间运行 |

## 🧪 测试

V2 配套了单元测试，覆盖配置加载、预警检查、数据提供者、Sheet Handler 等核心逻辑：
